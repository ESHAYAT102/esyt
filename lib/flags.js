...
export function mapNpmToLabel(token) {
  const map = {
    ...
    "@tanstack/react-form": "TanStack",
    "@tanstack/react-hotkeys": "TanStack",
  };
...
Packages
    ...
    Examples: --tailwindcss  --tanstack  -tailwindcss  --dotenv  -dotenv
...
Install / Git / Editor / Dev
    ...
    -nvim, -zed, -code, -cursor, -trae Select editor to open the project (Zed, VSCode, Cursor, Trae)
...
export function parseFlags(argv) {
  ...
  const strip = (t) =>
    t.startsWith("--") ? t.slice(2) : t.startsWith("-") ? t.slice(1) : t;
...
export function validateFlags(flags) {
  ...
  if (flags.selectedIDE !== undefined) {
    const allowed = ["Nvim", "Zed", "VSCode", "Cursor", "Trae", "None"];
